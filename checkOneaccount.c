...
void checkOneAccount(PGconn *conn, int user_id) {
  ...
  sprintf(query,
          ...
          "transactions.transaction_date, accounts.interest_rate FROM accounts "
  ...
  if (PQntuples(res) > 0) {
    ...
    float interest_rate = atof(PQgetvalue(res, 0, 3));
    ...
    if (strcmp(accountType, "savings") == 0 ||
      ...
      while (depositYear < currentYear ||
        ...
        interestForMonth =
            (balance * interest_rate) / 12; // Monthly interest for savings
      ...
      if (strcmp(accountType, "fixed01") == 0) {
        printw("You will gain this interest %0.2f on %d/%d/%d\n",
               interestForMonth * 12, depositDay, depositMonth,
               depositYear + 1);
      } else if (strcmp(accountType, "fixed02") == 0) {
        printw("You will gain this interest %0.2f on %d/%d/%d\n",
               interestForMonth * 24, depositDay, depositMonth,
               depositYear + 2);
      } else if (strcmp(accountType, "fixed03") == 0) {
        printw("You will gain this interest %0.2f on %d/%d/%d\n",
               interestForMonth * 36, depositDay, depositMonth,
               depositYear + 3);
      } else {
